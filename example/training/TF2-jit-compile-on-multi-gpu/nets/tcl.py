...
class Conv2d(tf.keras.layers.Layer):
    ...
    def __init__(self, kernel_size, num_outputs, strides = 1, dilations = 1, padding = 'SAME',
                 kernel_initializer = tf.keras.initializers.VarianceScaling(scale = 2., mode='fan_out', seed=seed),
                 use_biases = True,
    ...
    def build(self, input_shape):
        ...
        self.kernel = self.add_weight(name  = 'kernel', 
                                      ...
                                      trainable = self.trainable, dtype=self.dtype)
        if self.use_biases:
...
class DepthwiseConv2d(tf.keras.layers.Layer):
    ...
    def __init__(self, kernel_size, multiplier = 1, strides = [1,1,1,1], dilations = [1,1], padding = 'SAME',
                 kernel_initializer = tf.keras.initializers.VarianceScaling(scale = 2., mode='fan_in', seed=seed),
                 use_biases = True,
                 biases_initializer  = tf.keras.initializers.Zeros(),
                 activation_fn = None,
                 name = 'conv',
...
class FC(tf.keras.layers.Layer):
    ...
    def __init__(self, num_outputs, 
                 kernel_initializer = tf.keras.initializers.random_normal(stddev = 1e-2, seed=seed),
                 use_biases = True,
                 biases_initializer  = tf.keras.initializers.Zeros(),
                 activation_fn = None,
                 name = 'fc',
    ...
    def build(self, input_shape):
        ...
        self.ori_shape = self.kernel.shape
    def call(self, input):
        input = tf.cast(input, self.compute_dtype)
        kernel = self.kernel
...
class BatchNorm(tf.keras.layers.Layer):
    ...
    def _param_dtype(self):
        ...
        if self.dtype == tf.float16 or self.dtype == tf.bfloat16 or self.dtype == tf.cus:
            return tf.float32
        else:
    ...
    def build(self, input_shape):
        ...
        self.moving_mean = self.add_weight(name  = 'moving_mean', trainable = False,
                                      ...
                                      aggregation=tf.VariableAggregation.MEAN, dtype=self._param_dtype
                                      )
        ...
        if self.scale:
            self.gamma = self.add_weight(name  = 'gamma', 
                                         ...
                                         trainable = self.trainable, dtype=self._param_dtype)
        else:
            self.gamma = 1.
        if self.center:
    ...
    def call(self, input, training=None):
        ...
        # if inputs_dtype in (tf.float16, tf.bfloat16, tf.cus):
