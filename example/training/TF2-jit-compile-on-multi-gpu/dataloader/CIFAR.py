...
def build_dataset_providers(args, strategy, test_only = False):
    train_images, train_labels, test_images, test_labels, pre_processing = None, None, None, None, None
    ...
    train_ds = train_ds.shuffle(100*args.batch_size, seed=seed).batch(args.batch_size)
...
def Cifar10(args):
    ...
    def pre_processing(is_training = False):
        def training(image, *argv):
            ...
            image0 = tf.image.random_flip_left_right(image0, seed=seed)
            image0 = tf.pad(image0, [[4,4],[4,4],[0,0]], 'REFLECT')
            image0 = tf.image.random_crop(image0,sz, seed=seed)
