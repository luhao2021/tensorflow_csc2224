#ifndef TENSORFLOW_CORE_PLATFORM_EMULATION_FP16_H_
#define TENSORFLOW_CORE_PLATFORM_EMULATION_FP16_H_

#include "tensorflow/core/platform/emulation/base.h"

namespace fp16 {

CUS_DEVICE_FUNC static inline float fp32_from_bits(uint32_t w) {
...
CUS_DEVICE_FUNC static inline uint32_t fp32_to_bits(float f) {
#if defined(__OPENCL_VERSION__)
  return as_uint(f);
#elif defined(__CUDA_ARCH__)
...
CUS_DEVICE_FUNC static inline double fp64_from_bits(uint64_t w) {
#if defined(__OPENCL_VERSION__)
  return as_double(w);
#elif defined(__CUDA_ARCH__)
...
CUS_DEVICE_FUNC static inline uint64_t fp64_to_bits(double f) {
#if defined(__OPENCL_VERSION__)
  return as_ulong(f);
#elif defined(__CUDA_ARCH__)
 ...
 */
CUS_DEVICE_FUNC static inline uint32_t fp16_ieee_to_fp32_bits(uint16_t h) {
  /*
 ...
 */
CUS_DEVICE_FUNC static inline float fp16_ieee_to_fp32_value(uint16_t h) {
  /*
   * Extend the half-precision floating-point number to 32 bits and shift to the
   * upper part of the 32-bit word:
   *      +---+-----+------------+-------------------+
 ...
 * between integer and floating-point variables.
 */
CUS_DEVICE_FUNC static inline uint16_t fp16_ieee_from_fp32_value(float f) {
#if defined(__STDC_VERSION__) && (__STDC_VERSION__ >= 199901L) || \
    defined(__GNUC__) && !defined(__STRICT_ANSI__)
 ...
 * @note The implementation doesn't use any floating-point operations.
 */
CUS_DEVICE_FUNC static inline uint32_t fp16_alt_to_fp32_bits(uint16_t h) {
  /*
   * Extend the half-precision floating-point number to 32 bits and shift to the
   * upper part of the 32-bit word:
   *      +---+-----+------------+-------------------+
 ...
 * between integer and floating-point variables.
 */
CUS_DEVICE_FUNC static inline float fp16_alt_to_fp32_value(uint16_t h) {
  /*
   * Extend the half-precision floating-point number to 32 bits and shift to the
   * upper part of the 32-bit word:
   *      +---+-----+------------+-------------------+
 ...
 * between integer and floating-point variables.
 */
CUS_DEVICE_FUNC static inline uint16_t fp16_alt_from_fp32_value(float f) {
  const uint32_t w = fp32_to_bits(f);
  const uint32_t sign = w & UINT32_C(0x80000000);
  const uint32_t shl1_w = w + w;
...
#endif // TENSORFLOW_CORE_PLATFORM_EMULATION_FP16_H_