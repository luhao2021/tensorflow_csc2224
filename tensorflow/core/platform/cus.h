...
CUSTOM_DEVICE_FUNC static inline float fp32_from_bits(uint32_t w) {
...
CUSTOM_DEVICE_FUNC static inline uint32_t fp32_to_bits(float f) {
#if defined(__OPENCL_VERSION__)
  return as_uint(f);
#elif defined(__CUDA_ARCH__)
...
CUSTOM_DEVICE_FUNC static inline double fp64_from_bits(uint64_t w) {
#if defined(__OPENCL_VERSION__)
  return as_double(w);
#elif defined(__CUDA_ARCH__)
...
CUSTOM_DEVICE_FUNC static inline uint64_t fp64_to_bits(double f) {
#if defined(__OPENCL_VERSION__)
  return as_ulong(f);
#elif defined(__CUDA_ARCH__)
 ...
 */
CUSTOM_DEVICE_FUNC static inline uint32_t fp16_ieee_to_fp32_bits(uint16_t h) {
  /*
 ...
 */
CUSTOM_DEVICE_FUNC static inline float fp16_ieee_to_fp32_value(uint16_t h) {
  /*
   * Extend the half-precision floating-point number to 32 bits and shift to the
   * upper part of the 32-bit word:
   *      +---+-----+------------+-------------------+
 ...
 * between integer and floating-point variables.
 */
CUSTOM_DEVICE_FUNC static inline uint16_t fp16_ieee_from_fp32_value(float f) {
#if defined(__STDC_VERSION__) && (__STDC_VERSION__ >= 199901L) || \
    defined(__GNUC__) && !defined(__STRICT_ANSI__)
 ...
 * @note The implementation doesn't use any floating-point operations.
 */
CUSTOM_DEVICE_FUNC static inline uint32_t fp16_alt_to_fp32_bits(uint16_t h) {
  /*
   * Extend the half-precision floating-point number to 32 bits and shift to the
   * upper part of the 32-bit word:
   *      +---+-----+------------+-------------------+
 ...
 * between integer and floating-point variables.
 */
CUSTOM_DEVICE_FUNC static inline float fp16_alt_to_fp32_value(uint16_t h) {
  /*
   * Extend the half-precision floating-point number to 32 bits and shift to the
   * upper part of the 32-bit word:
   *      +---+-----+------------+-------------------+
 ...
 * between integer and floating-point variables.
 */
CUSTOM_DEVICE_FUNC static inline uint16_t fp16_alt_from_fp32_value(float f) {
  const uint32_t w = fp32_to_bits(f);
  const uint32_t sign = w & UINT32_C(0x80000000);
  const uint32_t shl1_w = w + w;
...
extern "C" cus_inline CUSTOM_DEVICE_FUNC uint32_t CastF32ToValue(const float f);
extern "C" cus_inline CUSTOM_DEVICE_FUNC float CastValueToF32(const uint32_t u);

struct cus {
  uint32_t value;

  CUSTOM_DEVICE_FUNC constexpr cus() : value(0) {}
  cus_inline explicit CUSTOM_DEVICE_FUNC cus(const float& f) : value(CastF32ToValue(f)) {}
  cus_inline explicit CUSTOM_DEVICE_FUNC cus(const double& d) : cus(static_cast<float>(d)) {}
  ...
  cus_inline CUSTOM_DEVICE_FUNC operator float() const { return CastValueToF32(value); }
...
extern "C" {
cus_inline CUSTOM_DEVICE_FUNC cus CusAdd(const cus a, const cus b);
cus_inline CUSTOM_DEVICE_FUNC cus CusSub(const cus a, const cus b);
cus_inline CUSTOM_DEVICE_FUNC cus CusMul(const cus a, const cus b);
cus_inline CUSTOM_DEVICE_FUNC cus CusDiv(const cus a, const cus b);

cus_inline CUSTOM_DEVICE_FUNC bool CusEq(const cus a, const cus b);
cus_inline CUSTOM_DEVICE_FUNC bool CusNe(const cus a, const cus b);
cus_inline CUSTOM_DEVICE_FUNC bool CusLt(const cus a, const cus b);
cus_inline CUSTOM_DEVICE_FUNC bool CusLe(const cus a, const cus b);
cus_inline CUSTOM_DEVICE_FUNC bool CusGt(const cus a, const cus b);
cus_inline CUSTOM_DEVICE_FUNC bool CusGe(const cus a, const cus b);
cus_inline CUSTOM_DEVICE_FUNC cus CusMax(const cus a, const cus b);

cus_inline CUSTOM_DEVICE_FUNC cus CusNeg(const cus a);
cus_inline CUSTOM_DEVICE_FUNC cus CusLog(const cus a);
cus_inline CUSTOM_DEVICE_FUNC cus CusExp(const cus a);
cus_inline CUSTOM_DEVICE_FUNC cus CusExpm1(const cus a);
cus_inline CUSTOM_DEVICE_FUNC cus CusLog1p(const cus a);
cus_inline CUSTOM_DEVICE_FUNC cus CusCos(const cus a);
cus_inline CUSTOM_DEVICE_FUNC cus CusSin(const cus a);
cus_inline CUSTOM_DEVICE_FUNC cus CusTanh(const cus a);
cus_inline CUSTOM_DEVICE_FUNC cus CusSqrt(const cus a);
cus_inline CUSTOM_DEVICE_FUNC cus CusRsqrt(const cus a);
cus_inline CUSTOM_DEVICE_FUNC cus CusCbrt(const cus a);
cus_inline CUSTOM_DEVICE_FUNC cus CusFloor(const cus a);
cus_inline CUSTOM_DEVICE_FUNC cus CusCeil(const cus a);
cus_inline CUSTOM_DEVICE_FUNC cus CusAbs(const cus a);
cus_inline CUSTOM_DEVICE_FUNC cus CusRoundNearestAfz(const cus a);
cus_inline CUSTOM_DEVICE_FUNC cus CusSign(const cus a);
cus_inline CUSTOM_DEVICE_FUNC cus CusIsFinite(const cus a);
}

cus_inline CUSTOM_DEVICE_FUNC cus operator+(const cus& a, const cus& b){ return CusAdd(a,b);}
cus_inline CUSTOM_DEVICE_FUNC cus operator-(const cus& a){ return CusNeg(a);}
cus_inline CUSTOM_DEVICE_FUNC cus operator-(const cus& a, const cus& b){ return CusSub(a,b);}
cus_inline CUSTOM_DEVICE_FUNC cus operator*(const cus& a, const cus& b){ return CusMul(a,b);}
cus_inline CUSTOM_DEVICE_FUNC cus operator/(const cus& a, const cus& b){ return CusDiv(a,b);}
cus_inline CUSTOM_DEVICE_FUNC cus& operator+=(cus& a, const cus& b) { 
  a = a + b;
  return a;
}
cus_inline CUSTOM_DEVICE_FUNC cus& operator-=(cus& a, const cus& b) { 
...
cus_inline CUSTOM_DEVICE_FUNC bool operator==(const cus& a, const cus& b){ return CusEq(a,b);}
cus_inline CUSTOM_DEVICE_FUNC bool operator<(const cus& a, const cus& b){ return CusLt(a,b);}
cus_inline CUSTOM_DEVICE_FUNC bool operator<=(const cus& a, const cus& b){ return CusLe(a,b);}
cus_inline CUSTOM_DEVICE_FUNC bool operator!=(const cus& a, const cus& b){ return CusNe(a,b);}
cus_inline CUSTOM_DEVICE_FUNC bool operator>(const cus& a, const cus& b){ return CusGt(a,b);}
cus_inline CUSTOM_DEVICE_FUNC bool operator>=(const cus& a, const cus& b){ return CusGe(a,b);}
...
CUSTOM_DEVICE_FUNC float CastValueToF32(const uint32_t u) {
  uint16_t v = static_cast<uint16_t>(u);
...
CUSTOM_DEVICE_FUNC cus CusMul(const cus a, const cus b) {
  ...
  // #define __HALF_TO_CUS(var) *(reinterpret_cast<const unsigned short
  // *>(&(var)))
  //   cus c;
  //   asm( "mul .f16 %0,%1,%2;\n"
  //     : "=h"(__HALF_TO_US(c)) : "h"(__HALF_TO_CUS(a)),"h"(__HALF_TO_CUS(b)));
  //   return c;
  // #else
  return cus(static_cast<float>(a) * static_cast<float>(b));
  // #endif
...
CUSTOM_DEVICE_FUNC bool CusEq(const cus a, const cus b) {
  return static_cast<float>(a) == static_cast<float>(b);
}
CUSTOM_DEVICE_FUNC bool CusNe(const cus a, const cus b) {
  return static_cast<float>(a) != static_cast<float>(b);
}
CUSTOM_DEVICE_FUNC bool CusLt(const cus a, const cus b) {
  return static_cast<float>(a) < static_cast<float>(b);
}
CUSTOM_DEVICE_FUNC bool CusLe(const cus a, const cus b) {
  return static_cast<float>(a) <= static_cast<float>(b);
}
CUSTOM_DEVICE_FUNC bool CusGt(const cus a, const cus b) {
  return static_cast<float>(a) > static_cast<float>(b);
}
CUSTOM_DEVICE_FUNC bool CusGe(const cus a, const cus b) {
  return static_cast<float>(a) >= static_cast<float>(b);
}
...
CUSTOM_DEVICE_FUNC cus CusRsqrt(const cus a) {
  // return cus(1.0f/std::sqrt(float(a)));
